	...
	foreach ( $wp_tables as $table ) {
		...
		if ( in_array( $table, $tables ) ) {
			...
			if ( defined('WP_REPAIRING') )
				return true;
			// Die with a DB error.
			$wpdb->error = sprintf( /*WP_I18N_NO_TABLES*/'تعدادی از جداول پایگاه‌داده در دسترس نمی‌باشند. پایگاه‌داده احتمالاً باید <a href="%s">تعمیر</a> شود.'/*/WP_I18N_NO_TABLES*/, 'maint/repair.php?referrer=is_blog_installed' );
			dead_db();
		}
	}
