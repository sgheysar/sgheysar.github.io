...
// Do not delete these lines
	...
	if ( post_password_required() ) { ?>
		<p class="nocomments"><?php _e('این یک مطلب خصوصی است پسورد را وارد کنید تا نظرات را ببینید.'); ?></p>
	<?php
		return;
	}
?>
<!-- You can start editing here. -->
	...
	<?php if ( have_comments() ) { ?>
			<div id="comments-div"><span id="comments-addcomment"><a href="#respond"  rel="nofollow" title="<?php _e('ارسال نظر'); ?>"><?php _e('ارسال نظر'); ?></a></span><h2 id="comments"><?php comments_number('0', '1', '%' );?> <?php _e('نظرات'); ?></h2></div>
	...
	<?php if ( ! comments_open() && !is_page() ) { ?>
			<div id="comments-div"><h2 id="comments"><?php _e('امکان ارسال نظر وجود ندارد.'); ?></h2></div>
	...
	$fields =  array(
			...
			'<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" />'. ' <label for="author"><small>' . __( 'نام' ) . '</small></label></p>',
			'email'  => '<p class="comment-form-email">' .
			'<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" /> <label for="email">' . __( 'ایمیل', 'zbench' ) . '</label></p>',
			'url'    => '<p class="comment-form-url">' .
			'<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /> <label for="url">' . __( 'وبسایت', 'zbench' ) . '</label></p>',
			);
	$args = array(
			...
			'title_reply'          => __( 'ارسال نظر'),
			'title_reply_to'       => __('پاسخ به %s &not;<br />'), 
			'cancel_reply_link'    => __( '<small>لغو</small>'),
			'label_submit'         => __( 'ارسال', 'zbench' )
			);
	...
	if($havepings == 1) : ?>
		<div class="trackbacks-pingbacks">
			<h3><?php _e('بازتاب‌ها و پینگ‌ها'); ?></h3>
			<ul id="pinglist">
